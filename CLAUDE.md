...
- ~~**Axe F — CSS par feature**~~ ✅ livré mai 2026 (~4 800 lignes migrées). Architecture : `src/styles/{tokens,base,layout}.css` global + chaque composant UI partagé et chaque feature ont leur propre `.css` co-localisé importé via leur `.tsx`. Stylelint configuré (`npm run lint:css`) avec `stylelint-config-standard` ; règles cosmétiques (1 décl/ligne, lightness-notation, etc.) désactivées pour respecter le mix existant. Reste à challenger : reformatage uniforme massif (1 décl/ligne) — pas urgent.
